#define OPTIMIZATION_NETWORK_MERGE
...
namespace ProtoBuf
{
    ...
    public sealed class ProtoReader : IDisposable
    {
...
#if OPTIMIZATION_NETWORK_MERGE
		public ProtoReader(SmartBuffer source, TypeModel model, SerializationContext context) 
		{
		...
		public ProtoReader(SmartBuffer source, TypeModel model, SerializationContext context, int length)
		{
			Init(this, source, model, context, length);
		}
		
		private static void Init(ProtoReader reader, SmartBuffer source, TypeModel model, SerializationContext context, int length)
		{
			if (source == null) throw new ArgumentNullException("source");
//			if (!source.CanRead) throw new ArgumentException("Cannot read from stream", "source");
			reader._destBuf = source;
		...
		internal void Ensure(int count, bool strict)
		{
			...
			{
//				Utility.LogError(string.Format("-----Ensure ---  count > ioBuffer.Length count:{0}, ioBuffer.Length:{1}",count, ioBuffer.Length));
				BufferPool.ResizeAndFlushLeft(ref ioBuffer, count, ioIndex, available);
				ioIndex = 0;
			}
			else if (ioIndex + count >= ioBuffer.Length)
			{
//				Utility.LogError(string.Format("-----Ensure --- ioIndex + count >= ioBuffer.Length ioIndex + count:{0},bufferlength:{1}",ioIndex + count,ioBuffer.Length));
...
//			Utility.LogError(string.Format("-----Ensure ---_destBuf --size:{0},writePos:{1},canRead:{2},buffer pos:{3}",_destBuf.Size,writePos,canRead,_destBuf.ReadPosition));
			while (count > 0 && canRead > 0 && (bytesRead = _destBuf._Read(ioBuffer, writePos, canRead)) > 0)
			{
//				Utility.LogError(string.Format("++++ bytesRead:{0},postion:{1}",bytesRead,_destBuf.ReadPosition));
//				if(bytesRead>=1015){
//					Utility.LogError("canRead>=1015");
//				}
				available += bytesRead;
				count -= bytesRead;
				canRead -= bytesRead;
...
#if OPTIMIZATION_NETWORK_MERGE
				ProtoReader.Seek(_destBuf, len, ioBuffer);
#else
		...
		#if OPTIMIZATION_NETWORK_MERGE
		internal static void Seek(SmartBuffer source, int count, byte[] buffer)
{

			{
				source.SkipRead((UInt32)count);
				count = 0;
			}
...
#region RECYCLER
		...
		internal static ProtoReader Create(SmartBuffer source, TypeModel model, SerializationContext context, int len)
		{
