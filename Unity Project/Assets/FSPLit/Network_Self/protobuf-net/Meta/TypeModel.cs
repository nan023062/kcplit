...
namespace ProtoBuf.Meta
{
    ...
    public abstract class TypeModel
    {
        ...
        /// </summary>
		public void Serialize(SmartBuffer dest, object value)
		{
        ...
        /// Writes a protocol-buffer representation of the given instance to the supplied stream.
        /// </summary>
		ProtoWriter __writer = new ProtoWriter ();
		public void Serialize(SmartBuffer dest, object value, SerializationContext context)
		{
			__writer.Reset(dest, this, context);
			__writer.SetRootObject(value);
			SerializeCore(__writer, value);
			__writer.Close();
		}
		...
		public object Deserialize(SmartBuffer source, object value, System.Type type)
		{
			return Deserialize(source, value, type, null);
		}
		...
		public object Deserialize(SmartBuffer source, object value, System.Type type, SerializationContext context)
		{
			bool autoCreate = PrepareDeserialize(value, ref type);
			ProtoReader reader = null;
        ...
        internal static Type GetListItemType(TypeModel model, Type listType)
        {
			...
			methods = listType.GetMethods();
			size = listType.GetMethods().Length;
            ...
            if(!isQueueStack)
            {
				...
				size = listType.GetInterfaces().Length;

				for(int i = 0;i< size;i++)
                {
					Type iType = listType.GetInterfaces()[i];
                    TestEnumerableListPatterns(model, candidates, iType);
                }
            }

            // more convenient GetProperty overload not supported on all platforms
			PropertyInfo[] _prolist=listType.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			size = _prolist.Length;
			for(int i = 0;i< size;i++)
            {
				PropertyInfo indexer = _prolist[i];
        ...
        private static object CreateListInstance(Type listType, Type itemType)
        {
                ...
                Helpers.GetConstructor(listType, Helpers.EmptyTypes, true) == null)
            {
                ...
                if (listType.IsInterface &&
                    (fullName = listType.FullName) != null && fullName.IndexOf("Dictionary") >= 0) 
                {
