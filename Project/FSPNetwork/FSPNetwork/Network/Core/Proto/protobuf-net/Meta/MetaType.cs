...
namespace ProtoBuf.Meta
{
    ...
    public class MetaType : ISerializerProxy
    {
        ...
        private IProtoTypeSerializer BuildSerializer()
        {
			...
			int size = fieldCount + subTypeCount;
            ...
            if (subTypeCount != 0)
            {
					BasicList.NodeEnumerator et =	subTypes.GetEnumerator();
//                foreach (SubType subType in subTypes)
					while(et.MoveNext())
                {

						SubType subType = (SubType)et.Current;
            ...
            if (fieldCount != 0)
            {
					BasicList.NodeEnumerator et =	fields.GetEnumerator();
//                foreach (ValueMember member in fields)
				while(et.MoveNext())
                {

						ValueMember member = (ValueMember)et.Current;
...
#endif
				int size  = foundList.Length;
//            foreach (MemberInfo member in foundList)
			for(int i = 0;i< size;i++)
            {
					MemberInfo member = (MemberInfo)foundList[i];
...
//            foreach (ProtoMemberAttribute normalizedAttribute in arr)
            ...
            {
				ProtoMemberAttribute normalizedAttribute = arr[i];
